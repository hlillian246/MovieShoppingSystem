function cartDelete() {
    ...
    $.ajax({
        ...
        url: GW_ENDPOINT + "/billing/cart/delete",
        headers: {
...
function fetchCartDeleteReport(count) {
    setTimeout(function () {
        $.ajax({
            ...
            if (xhr.status == 204) {
                if (count >= MAX_REPORT_FETCH_TRIES) {
                    document.getElementById("cartStatus").style.color = "red";
                    document.getElementById("cartStatus").innerHTML = "Timeout. Unable to fetch cart delete result.";
                    return false;
                } else {
                    fetchCartDeleteReport(count + 1);
                }
