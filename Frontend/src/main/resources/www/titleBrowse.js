...
function fetchTitleBrowseReport(count)
{
    setTimeout(function () {
        $.ajax({
            method: "GET", // Declare request type
...
//                    console.log(current)
                    ...
                    if (page === 1) {
                        result += '<tr><td>' + page + '</td>' +
                            '<td><a href="#next" onclick="titleSelectNext()" name="title">Next</a></td>';
                    } else {
                        result += '<tr><td><a href="#prev" onclick="titleSelectPrev()" value=title>Prev</a></td>' +
                            '<td>' + page + '</td>' +
                            '<td><a href="#next" onclick="titleSelectNext()" value=title>Next</a></td>';
                    }
...
function startTitleSearch(selected) {
    title = selected;
    ...
    $.cookie('search_type', 'title_browse');

    titleSelect();
}


function titleSelectNext() {
    console.log(title);
